...
**IOrder**
Данные заказа. Наследуется от `IContact`.
...
**IOrderForm**
Валидация формы заказа. Наследуется от `ContactsError` и `OrderError`.

**IOrderResult**
Ответ от сервера при заказе.
...
**IActionEvent**
Функция нажатия на кнопку и получение объекта события.
...
**IOrdersForm**
Форма заказа. Наследуется от `IState`.
